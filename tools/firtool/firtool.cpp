...
static debug::EmitUHDIOptions getUHDIOptions(StringRef verilogFileName = "") {
  debug::EmitUHDIOptions opts;
  opts.sourceFilePrefix = uhdiSourcePrefix;
  opts.outputFilePrefix = uhdiOutputPrefix;
  opts.verilogFileName = verilogFileName;
...
struct EmitUHDIPass
  ...
  llvm::raw_ostream &os;
  /// Path of the Verilog file the document describes; recorded verbatim.
  std::string verilogFileName;
  EmitUHDIPass(llvm::raw_ostream &os, StringRef verilogFileName)
      : os(os), verilogFileName(verilogFileName.str()) {}
  void runOnOperation() override {
    markAllAnalysesPreserved();
    if (failed(debug::emitUHDI(getOperation(), os,
                               getUHDIOptions(verilogFileName))))
      return signalPassFailure();
  }
};
...
static LogicalResult processBuffer(
  ...
  // `verilogFileName` seeds representations.verilog.files so the document
  // names a netlist even when no operation carries an emitted location. Split
  // emission has no single file to name -- the output option is a directory
  // there -- and its per-module locations already carry the real `.sv` names,
  // so it seeds nothing.
  auto addUHDIPass = [&](StringRef verilogFileName) -> LogicalResult {
    if (!emitUHDI)
      return success();
  ...
  if (outputFormat == OutputVerilog || outputFormat == OutputSplitVerilog ||
    ...
    case OutputVerilog:
      ...
      if (failed(addUHDIPass(firtoolOptions.getOutputFilename())))
        return failure();
      break;
    case OutputSplitVerilog:
      ...
      if (failed(addUHDIPass(/*verilogFileName=*/"")))
        return failure();
      break;
