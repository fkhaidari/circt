...
// Inline scope entry keyed by stable_id; it owns the scoped variable.
...
// The scoped variable itself; the scope owning it is checked through the
// inline scope's variableRefs above.
// CHECK:        "var_2222_0000":
// CHECK:          "name": "x"
...
// no instantiates). An extern-only design is emitted as-is: no `top` key and
// no synthetic scope standing in for one.
...
// Aggregate path: the constant lands on the synthetic member variable.
// Width is implicit in the bit-string length.
// CHECK:       "var_wide_aggr":
// CHECK-NEXT:    "bindKind": "node"
// CHECK-NEXT:    "memberRefs":
// CHECK-NEXT:      "var_wide_aggr__x"
// CHECK:       "var_wide_aggr__x":
// CHECK:         "verilog":
// CHECK-NEXT:      "value":
...
// each aggregate variable lists its members in `memberRefs`, and the
// variable's `typeRef` points at the outer struct.
...
// Variable: typeRef points at the outer struct; memberRefs list the
// synthetic member variables in `members` order, and the aggregate
// itself carries no verilog value.
// CHECK:       "variables":
// CHECK:         "var_5555_0000":
// CHECK-NEXT:      "bindKind": "node"
// CHECK-NEXT:      "memberRefs":
// CHECK-NEXT:        "var_5555_0000__in"
// CHECK-NEXT:        "var_5555_0000__out"
// CHECK:           "representations":
// CHECK:             "verilog": {}
// CHECK:           "typeRef": "Nested_io"

// Each leaf Variable names its own flat signal. A member that is itself
// an aggregate has no signal of its own and lists its members in turn.
// CHECK:         "var_5555_0000__in":
// CHECK-NEXT:      "bindKind": "synthetic"
// CHECK-NEXT:      "memberRefs":
// CHECK-NEXT:        "var_5555_0000__in__a"
// CHECK-NEXT:        "var_5555_0000__in__b"
// CHECK:             "verilog": {}
...
// CHECK:         "kind": "module"
// The covered input is listed once, through the real dbg.variable's id.
// CHECK:         "variableRefs":
// CHECK-NEXT:      "var_inter_c",
