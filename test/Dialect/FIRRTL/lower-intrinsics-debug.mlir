...
// A flipped bundle field is recorded on its `dbg.struct` in `dbg.flips`,
// relative to that struct. Lowering erases the bundle type, so this is the
// only record of the flip. A struct with no flipped field has no attribute.

// CHECK-LABEL: firrtl.module @FlipTest
firrtl.circuit "FlipTest" {
  firrtl.module @FlipTest(out %io: !firrtl.bundle<in flip: bundle<a: uint<8>, b: uint<8>>, out: uint<8>>) {
    // CHECK:      dbg.struct {"a": %{{.+}}, "b": %{{.+}}} :
    // CHECK:      dbg.struct {"in": %{{.+}}, "out": %{{.+}}} {dbg.flips = array<i1: true, false>}
    // A bundle with a flip is not passive, so the var finds the port by name.
    firrtl.int.generic "circt_debug_var"
      <name: none = "io", typeName: none = "IO[AnonymousBundle]"> : () -> ()
  }
}
