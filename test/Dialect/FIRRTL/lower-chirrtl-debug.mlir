...
// ---- Test 2: dbg.value referencing a CHIRRTL memoryport data result ----
// dbg.value (like dbg.variable) consumes an SSA value; its operand must
// also be remapped when it references a CHIRRTL port data result.

// CHECK-LABEL: firrtl.module @DebugValueMemPort
firrtl.circuit "DebugValueMemPort" {
  firrtl.module @DebugValueMemPort(in %clock: !firrtl.clock,
                                       in %addr: !firrtl.uint<8>) {
    %ram = chirrtl.combmem : !chirrtl.cmemory<uint<8>, 256>
    ...
    // dbg.value references the chirrtl memoryport data result (%rdata).
    // After the pass, the operand must be remapped; the value op must
    // survive with a valid (non-CHIRRTL) operand.
    // CHECK: %[[SF:.+]] = dbg.value %{{.*}} : !firrtl.uint<8>
    // CHECK: dbg.struct {"rdata": %[[SF]]}
    %sf = dbg.value %rdata : !firrtl.uint<8>
    %st = dbg.struct {"rdata": %sf} : !dbg.value
