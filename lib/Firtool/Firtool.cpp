...
populatePrepareForExportVerilog(mlir::PassManager &pm,
  ...
  // With --disable-optimization PrettifyVerilog is skipped, so the snapshot
  // sees unprettified names; EmitUHDI treats the live IR as authoritative,
  // the snapshot is a best-effort cache either way.
  if (opt.shouldEnableUhdi())
