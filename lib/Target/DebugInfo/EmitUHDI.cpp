...
// vector), variables (per-repr name/loc/value with sigName / constant /
// bitVector binding; aggregates list their members via memberRefs), scopes
// (modules + inline).
//
//===----------------------------------------------------------------------===//
...
/// receives one extra `variables[id]` entry per member, and the aggregate
/// lists those ids in `memberRefs`, so consumers see per-field
/// source-language metadata (typeName / enum typeRef) and the flat signal
/// of every leaf.
static Object emitVariable(
  ...
  // Verilog value: constant -> constant / bitVector; otherwise -> sigName via
  // sv::resolveVerilogName. An aggregate has no value of its own (its members
  // are listed in `memberRefs` below), and a scalar that lowering left as a
  // comb expression gets none either.
  Object verilog;
  ...
  if (!outputPortName.empty()) {
    ...
    // Bound through `memberRefs`, not to a single signal.
  ...
  // Each leaf carries a verilog repr naming its own flat signal; each
  // aggregate lists its members in `memberRefs`, in the order of the type's
  // members (struct) or indices (array). Entries are emitted parent-first
  // so the scope's variableRefs list a parent before its children.
  if (syntheticOut && isAggregate) {
    auto parentId = var->getAttrOfType<StringAttr>(kUhdiStableIdAttr);
    if (parentId) {
      std::function<std::string(StringRef, mlir::Value, const Twine &)>
          visitField;
      // Emit one synthetic Variable per member of `aggr`; returns their ids.
      auto visitMembers = [&](Operation *aggr, const Twine &pathPrefix) {
        Array refs;
        if (auto st = dyn_cast<debug::StructOp>(aggr))
          for (auto [nm, f] : llvm::zip(st.getNames(), st.getFields()))
            refs.push_back(
                visitField(cast<StringAttr>(nm).getValue(), f, pathPrefix));
        else if (auto ar = dyn_cast<debug::ArrayOp>(aggr))
          for (auto [idx, f] : llvm::enumerate(ar.getElements()))
            refs.push_back(visitField(std::to_string(idx), f, pathPrefix));
        return refs;
      };
      visitField = [&](StringRef bareName, mlir::Value fieldVal,
                       const Twine &pathPrefix) -> std::string {
        ...
        // A leaf that lowering left as one signal names it; a constant leaf
        // carries the literal. A value tied to neither gets no `value`.
        Object synVerilog;
        mlir::Value hwVal = unwrapToHwValue(innerVal);
        if (auto vname = leafName(hwVal)) {
          synVerilog["value"] = Object{{"sigName", vname.getValue().str()}};
        } else if (auto opR = dyn_cast<OpResult>(hwVal);
        ...
        size_t slot = syntheticOut->size();
        syntheticOut->emplace_back(synId, std::move(synEntry));
        ...
        if (auto opR = dyn_cast<OpResult>(innerVal);
            opR && isa<debug::StructOp, debug::ArrayOp>(opR.getOwner()))
          (*syntheticOut)[slot].second["memberRefs"] =
              visitMembers(opR.getOwner(), fullPath);
        return synId;
      };

      entry["memberRefs"] = visitMembers(rootVal.getDefiningOp(), Twine());
    }
  }
...
void UhdiEmitter::synthesizePortVars(
  ...
  for (size_t i = 0, e = modType.getNumPorts(); i < e; ++i) {
    ...
    reprs[kUhdiVerilogRepr] =
        Object{{"value", Object{{"sigName", portName.str()}}}};
    entry["representations"] = std::move(reprs);
...
void UhdiEmitter::collect(mlir::ModuleOp top) {
  ...
  for (auto &op : top.getOps()) {
    if (auto mod = dyn_cast<hw::HWModuleOp>(op)) {
      ...
      scopes[ext.getNameAttr().getValue().str()] =
          Object{{"kind", "extmodule"}};
    }
  }
