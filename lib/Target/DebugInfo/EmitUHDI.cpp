...
/// `bestVerilogName` resolve already and which names the wire the waveform
/// actually shows.
...
static Object emitVariable(
  ...
  if (!outputPortName.empty()) {
    ...
    haveValue = true;
  } else if (auto vname = bestVerilogName(var, unwrapToHwValue(rootVal))) {
  ...
  // A value the emitter could not tie to anything in the netlist is reported
  // as lost rather than omitted: an absent representation reads as "the
  // emitter did not look", which a consumer cannot tell from real loss.
  verilog["status"] = haveValue ? "preserved" : "lost";
...
/// Maps a variable name, within the scope it belongs to, onto the pool id
/// the document uses for it. Only roots are indexed: a reference into an
/// aggregate derives its id from the root's, by the same rule `emitVariable`
/// uses when it synthesises the per-field pool entries, so the two agree by
/// construction rather than by convention.
struct VarRefIndex {
  llvm::DenseMap<std::pair<mlir::Value, llvm::StringRef>, std::string> map;
};

static void addToIndex(VarRefIndex &idx, Operation *op, StringRef name,
                       mlir::Value scope, StringRef kind, hw::HWModuleOp mod) {
  auto id = op->getAttrOfType<StringAttr>(kUhdiStableIdAttr);
  if (!id || name.empty())
    return;
  auto [it, inserted] = idx.map.try_emplace({scope, name}, id.getValue().str());
  if (!inserted && it->second != id.getValue()) {
    op->emitWarning() << "uhdi: " << kind << " name '" << name
                      << "' collides within module '" << mod.getName()
                      << "'; references to it will be left unresolved";
    it->second = "";
  }
}

/// Resolve a structured reference to a pool id. Empty when the root names no
/// indexed variable.
static std::string resolveRef(const VarRefIndex &index, debug::VarRefAttr ref,
                              mlir::Value currentScope) {
  auto lookup = [&](mlir::Value scope) -> StringRef {
    auto it = index.map.find({scope, ref.getRoot().getValue()});
    if (it == index.map.end() || it->second.empty())
      return {};
    return it->second;
  };
  StringRef rootId = lookup(currentScope);
  // A variable declared without an explicit scope belongs to the module, and
  // is visible to statements that do carry one.
  if (rootId.empty() && currentScope)
    rootId = lookup(mlir::Value());
  if (rootId.empty())
    return {};

  // Same rule as `emitVariable`'s per-field ids: segments joined with `__`,
  // array indices spelled as decimal.
  std::string out = rootId.str();
  for (Attribute step : ref.getPath()) {
    out += "__";
    if (auto field = dyn_cast<StringAttr>(step))
      out += field.getValue();
    else if (auto index = dyn_cast<IntegerAttr>(step))
      out += std::to_string(index.getValue().getZExtValue());
    else
      return {};
  }
  return out;
...
/// Flatten the `hw.hierpath` a cross-module reference names into the dotted
/// path the document carries. Kept as a symbol in the IR until here, so it
/// survives the renaming a string would not.
static std::string resolveXmrPath(Operation *context, FlatSymbolRefAttr sym) {
  auto hierPath =
      SymbolTable::lookupNearestSymbolFrom<hw::HierPathOp>(context, sym);
  if (!hierPath)
    return {};
  std::string out;
  for (auto entry : hierPath.getNamepath()) {
    ...
    else if (auto flat = dyn_cast<FlatSymbolRefAttr>(entry))
      out += flat.getValue().str();
    else
...
/// One level of the condition stack a statement sits under.
struct GuardToken {
  std::string ref;
  bool negated = false;
};

/// The document carries the condition of a statement as one expression, so a
/// debugger evaluates it without walking back up the tree. The IR does not
/// store it: it is the conjunction of the enclosing subblocks, and keeping a
/// second copy is what previously let the two drift apart.
static Object emitBreakpointMeta(ArrayRef<GuardToken> guards) {
  if (guards.empty())
    return {};
  std::string enable;
  ...
  return Object{{"enableRef", enable}};
}
...
/// Document tokens for the two cases that name nothing. The IR spells these
/// as distinct attributes; they become text only here, on the way out.
static constexpr llvm::StringLiteral kComplexGuardToken = "<complex>";
static constexpr llvm::StringLiteral kConstSourceToken = "<const>";

/// Resolve one reference for the document, complaining if it names something
/// the pool does not have. Falling back silently is what let dangling
/// references reach consumers unannounced.
static std::string emitRef(Operation *stmt, StringRef role,
                           const VarRefIndex &index, debug::VarRefAttr ref,
                           mlir::Value currentScope) {
  std::string resolved = resolveRef(index, ref, currentScope);
  if (!resolved.empty())
    return resolved;
  stmt->emitWarning() << "uhdi: statement " << role << " names '"
                      << ref.getRoot().getValue()
                      << "', which resolves to no variable in the document; "
                         "emitting the name as written";
  return ref.getRoot().getValue().str();
}

static Array emitStatementList(Region &region, const VarRefIndex &varIndex,
                               EmitState &s, mlir::Value currentScope,
                               SmallVectorImpl<GuardToken> &guards) {
  Array body;
  if (region.empty())
    return body;
  for (Operation &op : region.front()) {
    ...
    if (auto connect = dyn_cast<debug::ConnectStmtOp>(op)) {
      entry["kind"] = "connect";
      entry["varRef"] = emitRef(&op, "destination", varIndex, connect.getDest(),
                                currentScope);

      std::string source;
      if (auto var = dyn_cast<debug::VarRefAttr>(connect.getSrc()))
        source = emitRef(&op, "source", varIndex, var, currentScope);
      else if (auto sym = dyn_cast<FlatSymbolRefAttr>(connect.getSrc()))
        source = resolveXmrPath(&op, sym);
      else
        source = kConstSourceToken.str();
      entry["valueRef"] = Object{{"varRef", std::move(source)}};

      if (auto bp = emitBreakpointMeta(guards); !bp.empty())
        entry["bp"] = std::move(bp);
    } else if (auto block = dyn_cast<debug::SubBlockOp>(op)) {
      entry["kind"] = "block";
      std::string condition;
      if (mlir::Value expr = block.getCondition()) {
        // The condition is an SSA handle on a materialised expression; its
        // pool id is stamped on the op itself.
        if (auto *defOp = expr.getDefiningOp())
          if (auto id = defOp->getAttrOfType<StringAttr>(kUhdiStableIdAttr))
            condition = id.getValue().str();
        if (condition.empty())
          condition = kComplexGuardToken.str();
      } else if (auto var = dyn_cast_or_null<debug::VarRefAttr>(
                     block.getConditionRefAttr())) {
        condition = emitRef(&op, "condition", varIndex, var, currentScope);
      } else {
        condition = kComplexGuardToken.str();
      }
      entry["guardRef"] = condition;
      if (block.getNegated())
        entry["negated"] = true;

      guards.push_back({condition, block.getNegated()});
      entry["body"] =
          emitStatementList(block.getBody(), varIndex, s, currentScope, guards);
      guards.pop_back();
    } else if (auto decl = dyn_cast<debug::DeclStmtOp>(op)) {
      entry["kind"] = "decl";
      entry["varRef"] =
          emitRef(&op, "target", varIndex, decl.getTarget(), currentScope);
    } else {
      continue;
    }
...
private:
  ...
  void synthesizePortVars(VarRefIndex &index, hw::HWModuleOp mod,
...
void UhdiEmitter::synthesizePortVars(
    VarRefIndex &index, hw::HWModuleOp mod,
    llvm::StringMap<Array> &orderedVarsByScope,
    const llvm::DenseSet<unsigned> &coveredPortIndices) {
  ...
  for (size_t i = 0, e = modType.getNumPorts(); i < e; ++i) {
    ...
    Object reprs;
    reprs[kChiselRepr] =
        Object{{"name", portName.str()}, {"status", "preserved"}};
    ...
    entry["representations"] = std::move(reprs);
    // A statement may name a port that no dbg.variable covers -- LowerTypes
    // scalarises an aggregate port into scalars the debug ops wrap as one
    // aggregate, leaving the raw ports named only here. Index the synthesised
    // entry so such a reference resolves to it instead of being emitted as a
    // bare name. `try_emplace` leaves an entry from a real dbg.variable in
    // place, since that one describes the port more precisely.
    index.map.try_emplace({mlir::Value(), portName}, varId);
...
void UhdiEmitter::collect(mlir::ModuleOp top) {
  ...
  top.walk([&](Operation *inner) {
    ...
    if (auto var = dyn_cast<debug::VariableOp>(inner)) {
      ...
      if (auto mod = var->getParentOfType<hw::HWModuleOp>()) {
        addToIndex(varRefIndexByMod[mod], var, var.getName(), var.getScope(),
                   "dbg.variable", mod);
        auto modType = mod.getHWModuleType();
    ...
    if (auto expr = dyn_cast<debug::ExpressionOp>(inner)) {
      ...
      if (auto mod = expr->getParentOfType<hw::HWModuleOp>())
        addToIndex(varRefIndexByMod[mod], expr, expr.getName(), expr.getScope(),
                   "dbg.expression", mod);
      return;
    }
  ...
  for (auto mod : top.getOps<hw::HWModuleOp>())
    synthesizePortVars(varRefIndexByMod[mod], mod, orderedVarsByScope,
                       coveredPortsByMod[mod]);
  ...
  for (auto mod : top.getOps<hw::HWModuleOp>()) {
    ...
    for (auto rb : rbsIt->second) {
      ...
      withScopeEntry(targetKey, [&](Object &obj) {
        SmallVector<GuardToken, 4> guards;
        Array body = emitStatementList(rb.getBody(), varRefIndexByMod[mod], s,
                                       rbScope, guards);
        if (!body.empty())
