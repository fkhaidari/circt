...
private:
  ...
  Value wrap(Value inner, StringRef parentPath) {
    ...
    if (it == leafMetaMap.end())
      return debug::ValueOp::create(rewriter, loc, inner, {}, {}).getResult();

    const LeafMeta &meta = it->second;
    if (meta.enumVariantsMap)
      inner = debug::EnumOp::create(rewriter, loc, inner, meta.enumTypeName,
                                    meta.enumVariantsMap, meta.enumFqn)
                  .getResult();

    return debug::ValueOp::create(rewriter, loc, inner, meta.typeName,
                                  meta.params)
        .getResult();
  }
  ...
  Value buildBundle(Value value, BundleType type, StringRef parentPath) {
    ...
    for (auto [index, element] : llvm::enumerate(type.getElements())) {
      ...
      SmallString<64> fieldPath;
      (Twine(parentPath) + "." + element.name.getValue()).toVector(fieldPath);
      if (auto dbgVal = build(subOp.getResult(), fieldPath)) {
        fields.push_back(dbgVal);
        names.push_back(element.name);
        flips.push_back(element.isFlip);
      } else if (warnAt) {
    ...
    auto result = debug::StructOp::create(rewriter, loc, fields,
                                          rewriter.getArrayAttr(names));
    if (llvm::is_contained(flips, true))
      result->setAttr(debug::kDbgFlipsAttr,
                      rewriter.getDenseBoolArrayAttr(flips));
    eraseUnused(subOps);
    return result;
  }
  ...
  Value buildVector(Value value, FVectorType type, StringRef parentPath) {
    ...
    for (std::size_t i = 0; i < type.getNumElements(); ++i) {
      ...
      SmallString<64> elemPath;
      (Twine(parentPath) + "[" + Twine(i) + "]").toVector(elemPath);
