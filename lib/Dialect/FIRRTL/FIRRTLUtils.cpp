...
static Value
convertToDebugAggregates(OpBuilder &builder, Value value, uint64_t fieldID,
                         llvm::function_ref<bool(uint64_t fieldID)> keepLeaf) {
  return FIRRTLTypeSwitch<Type, Value>(value.getType())
      .Case<BundleType>([&](auto type) {
        ...
        for (auto [index, element] : llvm::enumerate(type.getElements())) {
          ...
          if (auto dbgValue = convertToDebugAggregates(
                  builder, subOp, fieldID + type.getFieldID(index), keepLeaf)) {
        ...
        if (llvm::is_contained(flips, true))
          result->setAttr(debug::kDbgFlipsAttr,
                          builder.getDenseBoolArrayAttr(flips));
      ...
      .Case<FIRRTLBaseType>([&](auto type) -> Value {
        if (!type.isGround() || (keepLeaf && !keepLeaf(fieldID)))
          return {};
        return value;
      })
      .Default({});
...
Value circt::firrtl::convertToDebugAggregates(
    OpBuilder &builder, Value value,
    llvm::function_ref<bool(uint64_t fieldID)> keepLeaf) {
  return ::convertToDebugAggregates(builder, value, 0, keepLeaf);
}
